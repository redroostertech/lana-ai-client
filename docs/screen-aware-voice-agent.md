...
```text
overlay microphone → @voice realtime WebSocket
  ├─ server VAD + partial/final Whisper transcripts
  ├─ durable conversation / session / turn identity
  └─ finalized turns → Hermes durable runs
                    │ typed desktop-voice.v1 events
                    ▼
 Electron preview / clarify / approve
  ├─ answer or summary
...
Electron main owns authentication/entitlement checks, permissions, native target capture, confirmation, idempotency, notification, and execution. The context-isolated overlay streams Float32 microphone frames, renders partials, plays ordered Kokoro chunks, and stops playback immediately on barge-in. Hermes owns durable conversation state, research, governed entity tools, approvals, runs, and artifacts. The old buffered decision endpoint remains a feature-flag rollback only.

## Conversation model

There is one agent mode and one durable `conversation_id` that survives realtime reconnects. Backend turns, Hermes runs, approvals, artifacts, proposals, and execution results are correlated to that conversation. Electron persists the last conversation event sequence and requests a replay after reconnecting. Electron may render recent turns, but it is no longer the source of truth.
...
Audio-input hardware is checked before activation. A Mac without a microphone reports `MICROPHONE_UNAVAILABLE`; connecting a Bluetooth or USB microphone makes the next session eligible. Audio frames remain in memory and stream to the authenticated @voice socket; they are not written to disk.
...
- No wake word or signed-out/background activation. Once explicitly activated, VAD handles multiple conversational turns until the user closes or cancels the session.
...
npx jest tests/unit/screen-voice-core.test.js \
  ...
  tests/unit/screen-voice-realtime-contract.test.js \
...
# Backend
npx jest tests/unit/voice/desktop-hermes-turn-handler.test.js \
  tests/unit/voice/voice-realtime.gateway.test.js \
  tests/unit/voice/voice.routes.test.js --runInBand
```

No new client API key is required. Enable `DESKTOP_VOICE_REALTIME_HERMES_ENABLED`, `HERMES_RUNTIME_ADAPTER_ENABLED`, and `HERMES_RUNTIME_APPROVAL_GATED_PILOT_ENABLED`; the backend reuses its existing Hermes, Whisper, and Kokoro deployment.
...
- [ ] Activate the agent, speak naturally, observe partial text, and let server VAD finalize without a shortcut release.
- [ ] Speak over Kokoro playback; playback stops immediately and the Hermes run remains queryable.
...
- Cross-device rendering of the durable conversation is not yet exposed in the overlay UI.
